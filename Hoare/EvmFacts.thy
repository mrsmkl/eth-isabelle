...
definition all :: "'a state_element set_pred" where
"all s = True"

definition ex :: "('a \<Rightarrow> 'b set_pred) \<Rightarrow> 'b set_pred" where
"ex f s = (\<exists>x. f x s)"

definition gas_smaller :: "int \<Rightarrow> 'a state_element set_pred" where
"gas_smaller x s = (\<exists>y. y < x \<and> gas_pred y s)"

definition some_gas :: "'a state_element set_pred" where
...
lemma sep_add_assoc : "((a \<or>* b) \<or>* c) = (a \<or>* (b \<or>* c))"
by (simp add:sep_add_def)

lemma sep_add_commute : "(a \<or>* b) = (b \<or>* a)"
unfolding sep_add_def
by blast
...
apply (auto simp: sep_conj_def sep_add_def)
  by blast


definition action ::
   "(contract_action \<Rightarrow> bool) \<Rightarrow> 'a state_element set_pred" where
"action p s = (\<exists>x. {ContractActionElm x} = s \<and> p x)"

definition failing :: "'a state_element set_pred" where
"failing = action (\<lambda>y. \<exists>x. ContractFail x = y)"

definition returning :: "'a state_element set_pred" where
"returning = action (\<lambda>y. \<exists>x. ContractReturn x = y)"

definition destructing :: "'a state_element set_pred" where
"destructing = action (\<lambda>y. \<exists>x. ContractSuicide x = y)"

definition calling :: "'a state_element set_pred" where
"calling = action (\<lambda>y. \<exists>x. ContractCall x = y)"

definition creating :: "'a state_element set_pred" where
"creating = action (\<lambda>y. \<exists>x. ContractCreate x = y)"

definition delegating :: "'a state_element set_pred" where
...
blockedInstructionContinue_def sha3_def

lemmas split_inst = arith_inst.splits bits_inst.splits
  sarith_inst.splits
  info_inst.splits
  memory_inst.splits
  storage_inst.splits
...
"instruction_sem v c aa net \<noteq>
 InstructionToEnvironment (ContractFail []) a b"
apply (auto simp add:rw Let_def split:if_splits option.splits
  inst.splits pc_inst.splits)
apply (auto simp:rw Let_def split:split_inst)
done
...
apply(auto simp add:rw not_continuing_def all_def
...
by (auto simp add:instruction_sem_simps Let_def split:if_splits option.splits
  inst.splits bits_inst.splits list.splits
   arith_inst.splits sarith_inst.split_asm
info_inst.splits memory_inst.split_asm
storage_inst.split_asm log_inst.split_asm stack_inst.split_asm
misc_inst.split_asm pc_inst.split_asm)
...
proof -
  have "meter_gas a v1 co_ctx net > 0"
   ...
   by blast
  then show ?thesis using continue_meter_gas and a by fastforce
qed

definition all_stack :: "'a state_element set_pred" where
"all_stack s = (\<exists>t. s = stack_as_set t)"

definition all_but_gas :: "'a state_element set_pred" where
...
lemma maximality_stack_aux2 :
 "stack_as_set s1 \<subseteq> stack_as_set s2 \<Longrightarrow>
  rev s1 = rev s2"
apply (rule list_eq)
using maximality_stack_length [of s1 s2]
apply fastforce
by (metis (no_types, lifting) diff_Suc_less length_greater_0_conv maximality_stack_aux maximality_stack_length nth_rev_alt)
...
lemma maximality_data_sent :
 "data_sent_as_set s1 \<subseteq> data_sent_as_set s2 \<Longrightarrow>
  s1 = s2"
apply (rule List.nth_equalityI)
using maximality_data_length [of s1 s2]
apply fastforce
apply auto
subgoal for i
...
lemma data_elem_at_context :
 "(SentDataElm (x,y) :: 'a state_element) \<in> contexts_as_set v2 c2 \<Longrightarrow>
  (SentDataElm (x,y) :: 'a state_element) \<in> data_sent_as_set (vctx_data_sent v2)"
by(simp add:context_rw)

lemma data_at_context :
 "(data_sent_as_set s1 :: 'a state_element set) \<subseteq> contexts_as_set v2 c2 \<Longrightarrow>
  (data_sent_as_set s1 :: 'a state_element set) \<subseteq> data_sent_as_set (vctx_data_sent v2)"
apply (auto)
...
lemma block_from_context :
 "blockhash_as_elm s1 \<subseteq> contexts_as_set v2 c2 \<Longrightarrow>
  s1 = block_blockhash (vctx_block v2)"
apply (rule maximality_block [of s1 " block_blockhash (vctx_block v2)"])
using block_at_context [of s1 v2 c2]
by simp

(* program *)

definition program_inst :: "int program \<Rightarrow> int \<Rightarrow> inst" where
"program_inst p i =
   (case program_content p i of None \<Rightarrow> Misc STOP | Some i \<Rightarrow> i)"

definition program_as_set2 :: "int program \<Rightarrow> int state_element set" where
"program_as_set2 p =
...
lemma log_elem_at_context :
 "(LogElm (x,y) :: 'a state_element) \<in> contexts_as_set v2 c2 \<Longrightarrow>
  (LogElm (x,y) :: 'a state_element) \<in> log_as_set (vctx_logs v2)"
apply(simp add:context_rw)
done

lemma log_at_context_aux :
 "(log_as_set s1 :: 'a state_element set) \<subseteq> contexts_as_set v2 c2 \<Longrightarrow>
  (x :: 'a state_element) \<in> log_as_set s1 \<Longrightarrow>
...
lemma log_at_context :
 "(log_as_set s1 :: 'a state_element set) \<subseteq> contexts_as_set v2 c2 \<Longrightarrow>
  (log_as_set s1 :: 'a state_element set) \<subseteq> log_as_set (vctx_logs v2)"
using log_at_context_aux [of s1 v2 c2] by auto
...
lemma maximality :
   "(contexts_as_set v1 c1 :: int state_element set) \<subseteq> contexts_as_set v2 c2 \<Longrightarrow>
    (contexts_as_set v1 c1 :: int state_element set) = contexts_as_set v2 c2"
apply (rule eq_class)
apply(simp add:context_rw)
apply(simp add:context_rw)
apply(simp add:context_rw)
...
apply(simp add:context_rw)
using block_from_context [of _ v2 c2]
  and block_subset [of v1 c1]
apply force
apply(simp add:context_rw)
apply(simp add:context_rw)
apply(simp add:context_rw)
...
by (auto simp add:zero_mul triple_def zero_def)

lemma loop_triple :
  "(\<forall>x. \<exists>y. y < x \<and> triple net {} (p x) {} (p y \<or>* q)) \<Longrightarrow>
...
lemma triple_three :
  "triple net {} p2 {} (q \<or>* p1) \<Longrightarrow>
   triple net {} p1 {} (q \<or>* p0) \<Longrightarrow>
   triple net {} p2 {} (q \<or>* p0)"
...
apply(drule_tac x = stopper in spec)
apply auto
  by (metis sep_add_def sep_add_distr sep_conj_commute)

lemma loop_triple2 :
  "(\<forall>x. triple net {} (p x) {} (p (x-1) \<or>* q)) \<Longrightarrow>
    triple net {} (p x) {} (p (0::nat) \<or>* q)"
apply (induction x)
apply auto
...
apply auto
using triple_three
  by (metis EvmFacts.sep_add_commute)

lemma loop_triple_int :
  "(\<forall>x. triple net {} (p x) {} (p (x-1) \<or>* q)) \<Longrightarrow>
    \<exists>y. y < (0::int) \<and> triple net {} (p x) {} (p y \<or>* q)"
apply (induction x)
subgoal for n
apply (induction n)
  apply force
...
definition stable :: "network \<Rightarrow> 'a state_element set_pred \<Rightarrow> bool" where
"stable net pre ==
...
lemma triple_stable :
   "triple net {} p {} (q \<or>* r) \<Longrightarrow>
    triple net {} (p \<or>* r) {} (q \<or>* r)"
  using triple_tauto triple_three
by (metis EvmFacts.sep_add_commute)

lemma triple_stable2 :
   "triple net {} p {} (q \<or>* s) \<Longrightarrow>
    triple net {} s {} r \<Longrightarrow>
    triple net {} (p \<or>* r) {} (q \<or>* r)"
  by (metis sep_add_def triple_stable triple_three weaken_post)

lemma loop_triple_int2 :
  "(\<forall>x. triple net {} (p x) {} (p (x-1) \<or>* q x)) \<Longrightarrow>
   (\<forall>x. triple net {} (q x) {} (q (x+1))) \<Longrightarrow>
   \<exists>y. y < (0::int) \<and> triple net {} (p x) {} (p y \<or>* q x)"
apply (induction x)
subgoal for n
apply (induction n)
...
lemma loop_triple_int3 :
  "(\<forall>x. triple net {} (p x) {} (p (x-1) \<or>* q x)) \<Longrightarrow>
   (\<forall>x. triple net {} (q x) {} (q (x+1))) \<Longrightarrow>
   \<exists>y. y < (0::int) \<and> y \<le> x \<and> triple net {} (p x) {} (p y \<or>* q x)"
apply (induction x)
subgoal for n
apply (induction n)
  apply (smt semiring_1_class.of_nat_simps(1))
...
definition s_triple :: "network \<Rightarrow> 'a state_element set_pred \<Rightarrow> 'a state_element set_pred \<Rightarrow> bool" where
...
definition failures :: "failure_reason set \<Rightarrow> 'a state_element set_pred" where
